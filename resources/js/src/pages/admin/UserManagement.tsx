...
export function UserManagement({ currentUserEmail, onLogout, mode = 'active', onNavigate }: UserManagementProps) {
  ...
  const [createMiddleInitial, setCreateMiddleInitial] = useState('');
  const [createEmail, setCreateEmail] = useState('');
  const [createPasswordValue, setCreatePasswordValue] = useState('');
  const [createShowPassword, setCreateShowPassword] = useState(false);
  const [createRole, setCreateRole] = useState<'Employee' | 'Instructor'>('Employee');
  const [createDepartment, setCreateDepartment] = useState('');
  const [createSubdepartment, setCreateSubdepartment] = useState('');
  const [createSubdepartmentIds, setCreateSubdepartmentIds] = useState<number[]>([]);
  const [createIsHead, setCreateIsHead] = useState(false);
  const [createEmailAutoGenerated, setCreateEmailAutoGenerated] = useState(true);
  const [createFormError, setCreateFormError] = useState<string | null>(null);
  const [createSubmitting, setCreateSubmitting] = useState(false);
  const [createProfilePictureFile, setCreateProfilePictureFile] = useState<File | null>(null);
  const [createProfilePicturePreview, setCreateProfilePicturePreview] = useState<string | null>(null);
  ...
  */
  ...
  // ── Create New User modal helpers ──────────────────────────────────────────
  const openCreateUserModal = () => {
    setCreateFirstName('');
    setCreateLastName('');
    setCreateMiddleInitial('');
    setCreateEmail('');
    setCreateEmailAutoGenerated(true);
    setCreatePasswordValue('');
    setCreateShowPassword(false);
    setCreateRole('Employee');
    setCreateDepartment('');
    setCreateSubdepartment('');
    setCreateSubdepartmentIds([]);
    setCreateIsHead(false);
    setCreateFormError(null);
    setCreateProfilePictureFile(null);
    setCreateProfilePicturePreview(null);
    setIsCreateUserModalOpen(true);
  };

  const closeCreateUserModal = () => setIsCreateUserModalOpen(false);

  // Auto-generate email: {firstname}{lastnameabbrev}@maptech.com
  // Last name abbrev = first letter + last consonant before the final vowel
  // e.g. Delapaz → d(first) + p(consonant before last 'a') = "dp"
  useEffect(() => {
    if (!createEmailAutoGenerated) return;
    const first = createFirstName.trim().toLowerCase().replace(/\s+/g, '');
    const lastClean = createLastName.trim().toLowerCase().replace(/\s+/g, '');
    if (!first && !lastClean) { setCreateEmail(''); return; }
    ...
    const getAbbrev = (name: string) => {
      ...
      for (let i = name.length - 1; i >= 0; i--) {
        if (vowels.has(name[i])) { lastVowelIdx = i; break; }
      }
      if (lastVowelIdx <= 0) return name.slice(0, 2);
      for (let i = lastVowelIdx - 1; i >= 1; i--) {
        if (!vowels.has(name[i])) return ch + name[i];
      }
      return ch;
    };
    const base = `${first}${getAbbrev(lastClean)}`;
    const domain = '@maptech.com';
    const existingEmails = safeArray(users).map(u => u.email.toLowerCase());
    ...
    setCreateEmail(candidate);
  }, [createFirstName, createLastName, createEmailAutoGenerated]);

  const handleCreateUserSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setCreateFormError(null);

    if (!createLastName.trim() || !createFirstName.trim()) {
    ...
    if (createRole === 'Employee' && !createDepartment) {
      setCreateFormError('Department is required for Employee role');
      return;
    }
    if (createRole === 'Employee' && !createSubdepartment) {
      setCreateFormError('Subdepartment is required for Employee role');
      return;
    }

    const mi = createMiddleInitial.trim();
    const miFormatted = mi ? ' ' + mi.toUpperCase().split('').join('.') + '.' : '';
    const fullName = `${createLastName.trim()}, ${createFirstName.trim()}${miFormatted}`;
    ...
    try {
      ...
      const body: any = {
        ...
        subdepartment_id: createRole === 'Employee' && createSubdepartment ? Number(createSubdepartment) : null,
        status: 'Active',
      };
      if (createRole === 'Instructor') {
        body.subdepartment_ids = createSubdepartmentIds;
        body.is_department_head = createIsHead;
      }
      ...
      if (createProfilePictureFile) {
        const userId = responseData.user?.id;
        if (userId) {
          const xsrfToken2 = await getXsrfToken();
          const photoFormData = new FormData();
          photoFormData.append('profile_picture', createProfilePictureFile);
          await fetch(`${API_BASE}/admin/users/${userId}/photo`, {
            method: 'POST',
            credentials: 'include',
            headers: { 'Accept': 'application/json', 'X-XSRF-TOKEN': xsrfToken2 },
    ...
    <div className="space-y-6 ui-pop-grid um-shell">
      ...
      {isCreateUserModalOpen && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/75 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-slate-200 dark:border-slate-700">
            <form onSubmit={handleCreateUserSubmit} autoComplete="off">
              <div className="px-6 pt-6 pb-4 space-y-4">
                {/* Header */}
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Create New User</h3>
                  <button type="button" onClick={closeCreateUserModal} className="rounded-md p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                    <X className="h-5 w-5" />
                  </button>
                </div>

                {createFormError && (
                  <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-md p-3 flex items-center">
                    <AlertCircle className="h-4 w-4 text-red-500 mr-2 shrink-0" />
                    <span className="text-sm text-red-700 dark:text-red-300">{createFormError}</span>
                  </div>
                )}
                ...
                <div className="flex flex-col items-center py-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        const reader = new FileReader();
                        reader.onloadend = () => setCreateProfilePicturePreview(reader.result as string);
                        reader.readAsDataURL(file);
                      }
                    }}
                  />
                  <p className="text-xs text-slate-400 dark:text-slate-300 mt-2">
                    {createProfilePicturePreview ? 'Click photo to change' : 'Click to upload photo'}
                  </p>
                </div>
                ...
                {/* Full name preview */}
                {(createLastName.trim() || createFirstName.trim()) && (
                  <div className="px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700">
                    <p className="text-xs text-slate-500 dark:text-slate-400">Full name preview:</p>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                      {createLastName.trim()}
                      {createLastName.trim() && createFirstName.trim() ? ', ' : ''}
                      {createFirstName.trim()}
                      {createMiddleInitial.trim() ? ' ' + createMiddleInitial.trim().toUpperCase().split('').join('.') + '.' : ''}
                    </p>
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    ...
                    value={createEmail}
                    onChange={(e) => {
                      setCreateEmailAutoGenerated(false);
                      setCreateEmail(e.target.value);
                    }}
                    autoComplete="off"
                    className={modalFieldClass}
                  />
                  {createEmailAutoGenerated && createEmail && (
                    <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Auto-generated — edit to use a different email</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      autoComplete="new-password"
                      className={`${modalFieldClass} pr-10`}
                    />
                    <button
                      type="button"
                    ...
                    <div className="mt-2">
                      <div className="flex gap-1 mb-1">
                        {[1, 2, 3, 4].map((level) => {
                          const strength = (() => {
                            let s = 0;
                            if (createPasswordValue.length >= 8) s++;
                            if (createPasswordValue.length >= 12) s++;
                            if (/[A-Z]/.test(createPasswordValue) && /[a-z]/.test(createPasswordValue)) s++;
                            if (/[0-9]/.test(createPasswordValue)) s++;
                            if (/[^A-Za-z0-9]/.test(createPasswordValue)) s++;
                            return Math.min(s, 4);
                          })();
                          const color = strength <= 1 ? 'bg-red-500' : strength === 2 ? 'bg-orange-500' : strength === 3 ? 'bg-yellow-500' : 'bg-green-500';
                          return <div key={level} className={`h-1 flex-1 rounded-full transition-colors ${level <= strength ? color : 'bg-slate-200 dark:bg-slate-700'}`} />;
                        })}
                      </div>
                    </div>
                  )}
                ...
                {/* Role — card radio buttons */}
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">
                    Role <span className="text-red-500">*</span>
                  </label>
                  <div className="grid grid-cols-2 gap-3">
                    {(['Employee', 'Instructor'] as const).map((role) => (
                      <button
                        key={role}
                        type="button"
                        onClick={() => {
                      ...
                      >
                        <div>
                          <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{role}</div>
                ...
                <div>
                  ...
                  <div className="relative ui-select-wrap">
                    <select
                      ...
                      className={`${modalSelectClass} ui-select-custom-arrow`}
                    >
                      <option value="">Select department</option>
                      {departments.map((d) => (
                        <option key={d.id} value={d.name}>{d.name}</option>
                ...
                {createRole === 'Employee' && createDepartment && (
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
                      Subdepartment <span className="text-red-500">*</span>
                    </label>
                    <div className="relative ui-select-wrap">
                      <select
                        value={createSubdepartment}
                        onChange={(e) => setCreateSubdepartment(e.target.value)}
                        className={`${modalSelectClass} ui-select-custom-arrow`}
                      >
                        <option value="">Select subdepartment</option>
                ...
                {createRole === 'Instructor' && createDepartment && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const vals = Array.from(e.target.selectedOptions).map((o) => Number(o.value));
                        setCreateSubdepartmentIds(vals);
                      }}
                      className={`${modalFieldClass} h-28`}
                    >
                      {(departments.find((d) => d.name === createDepartment)?.subdepartments || []).map((s) => (
                    ...
                    <div className="flex items-center mt-2">
                      <input
                        type="checkbox"
                        checked={createIsHead}
                        onChange={(e) => setCreateIsHead(e.target.checked)}
                        className="h-4 w-4 text-green-600 focus:ring-green-500 border-slate-300 rounded"
                      />
                      <label className="ml-2 block text-sm text-slate-900 dark:text-slate-100">Set as Department Head</label>
                    </div>
